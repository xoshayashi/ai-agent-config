...
- **正本（Single source of truth）:** Act デザインガイドライン v1.2（Forest Charcoal + Petrol + Honey 体系）
...
- **Surface（クリーム `#ECE9E1`）と Ink（Forest Charcoal `#2D332E`）が主役**（合計で 80–94%）。純黒 `#0E0E08` は使わない。
...
- **Accent = Honey `#ECC85A`**。透明感と十分なコントラストを保つため、アクセントはこの色に統一する。
...
### やってはいけない配色
- 純黒 `#0E0E08` をテキストや面色として使う（Forest Charcoal `#2D332E` を使う）。
- Mustard `#D9B441` を Light mode で使う。
...
- Petrol 塗りロゴは使える。Honey 塗りは禁止（シグナル色は塊で使わない）。
- グラデ・アウトライン化・拡縮歪みも禁止。
