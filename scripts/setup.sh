...
copilot_home=$(abs_dir "${AI_AGENT_COPILOT_HOME:-$HOME/.copilot}")
...
write_state_config() {
...

  {
    ...
    printf 'AI_AGENT_COPILOT_HOME=%s\n' "$(quote_sh "$copilot_home")"
...
install_instruction_links() {
  ...
  install_link "$src_root/COPILOT.md" "$copilot_home/copilot-instructions.md"
  install_link "$src_root/AI_AGENT_INSTRUCTIONS.md" "$copilot_home/AI_AGENT_INSTRUCTIONS.md"
  install_link "$src_root/DESIGN.md" "$copilot_home/DESIGN.md"
  install_link "$src_root/HOOKS.md" "$copilot_home/HOOKS.md"
}
...
install_hook_dir_links() {
  src="$config_home/hooks"
  install_link "$src" "$claude_home/hooks"
  install_link "$src" "$codex_home/hooks"
  install_link "$src" "$gemini_home/hooks"
  install_link "$src" "$copilot_home/hooks"
}
...
install_hook_links() {
  ...
  install_hook_config "$src_root/copilot/settings.json" "$copilot_home/settings.json" json
}
...
if [ "$install_skills" = "1" ]; then
  ...
  install_skills_to_dir "$copilot_home/skills"
