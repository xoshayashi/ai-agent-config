...
schedule_update_after_setup=${AI_AGENT_SCHEDULE_UPDATE_AFTER_SETUP:-1}
schedule_skill_improvement_after_setup=${AI_AGENT_SCHEDULE_SKILL_IMPROVEMENT_AFTER_SETUP:-1}
...
case "$schedule_update_after_setup" in
  0|1) ;;
  *) fail "AI_AGENT_SCHEDULE_UPDATE_AFTER_SETUP must be 0 or 1" ;;
esac

case "$schedule_skill_improvement_after_setup" in
  0|1) ;;
  *) fail "AI_AGENT_SCHEDULE_SKILL_IMPROVEMENT_AFTER_SETUP must be 0 or 1" ;;
esac
...
configure_automation() {
  if [ "$schedule_update_after_setup" = "1" ]; then
    say "automation: configure update scheduler"
    run env \
      "AI_AGENT_CONFIG_HOME=$config_home" \
      "AI_AGENT_STATE_DIR=$state_dir" \
      "AI_AGENT_DRY_RUN=$dry_run" \
      sh "$config_home/scripts/schedule-update.sh"
  else
    say "skip: update scheduler disabled"
  fi

  if [ "$schedule_skill_improvement_after_setup" = "1" ]; then
    say "automation: configure skill-improvement scheduler"
    run env \
      "AI_AGENT_CONFIG_HOME=$config_home" \
      "AI_AGENT_STATE_DIR=$state_dir" \
      "AI_AGENT_DRY_RUN=$dry_run" \
      sh "$config_home/scripts/schedule-skill-improvement.sh"
  else
    say "skip: skill-improvement scheduler disabled"
  fi
}
...
if [ "$install_hooks" = "1" ]; then
  say "note: Hooks are installed in minimal mode. Managed hooks are limited to deterministic safety checks."
fi
