...
このリポジトリは、Claude Code / Codex のグローバル instruction files、共有 skill links、シェル設定 (`~/.zshrc`) を管理します。
...
`setup.sh` は Claude Code / Codex が未インストールでも止まりません。
...
- `pipx`
- `graphviz`（スライド skill の図解に必要な `dot`）
- `poppler`（スライド skill のレンダ検証に必要な `pdftoppm`）
...
Claude Code は公式推奨 of Native Install で入れます。既存の Homebrew
...
`chatgpt`、`claude`、`displaylink`、`docker-desktop`、`google-drive`、`tailscale-app`、`zoom` は installer や symlink
作成で管理者権限を要求します（`codex` と `claude` の CLI 本体は `~/.local/bin` への
公式 installer で入るため sudo は不要です）。
...
`setup.sh` は `gh`、`gcloud`、`hf` に加え、このリポジトリが設定する主役 CLI である
Claude Code / Codex のログイン、および git のコミット著者情報の設定状態を確認します。
未ログイン／未設定の場合、認証そのものは自動実行せず（資格情報はリポジトリに置けないため）、
次のような手動コマンドを表示します。
...
さらに実行の最後に、リポジトリが原理的に自動化できない一回きりの手動ステップ
（各アプリのサインイン、macOS のプライバシー権限付与、Apple ID / iCloud / App Store /
Wi-Fi / Keychain）を「remaining one-time steps」として一覧表示します。何が残るかを
必ず可視化するため、`setup.sh` の完了メッセージは「setup complete」と断定せず、
警告行の確認と `sh scripts/health-check.sh` での検証を促します。
...
| `fail` | 既存 path があれば失敗 |
| `replace` | 既存 path を backup folder へ移してから link 作成。skill は `~/.codex/skill-backups/`、`~/.claude/skill-backups/`、旧 `~/.agents/skill-backups/` に逃がし、`skills/` 直下には置きません。 |
...
`~/.claude/skills/<name>` にリンクします。skill が
...
| `python-pptx` / `Pillow` / `lxml` / `fonttools` / `matplotlib` / `matplotlib-venn` / `graphviz`(py) / `resvg_py` | 必須 | `act-structured-slide-generation` の pptx 生成・図表・検証 | `setup.sh` が `requirements.txt` からユーザー site-packages へ導入 |
| Geist / Noto Sans JP フォント（400/600/700） | 必須 | スライドの文字採寸（無いと折返しが近似化し描画が崩れる） | `setup.sh` が同梱の `setup_fonts.py` で `~/Library/Fonts` へ導入 |
| Graphviz CLI (`dot`) | 必須 | 図解（org_tree / node_graph）のビルド | `setup.sh` が `brew install graphviz` |
| poppler (`pdftoppm`) | 必須 | スライドのレンダリング検証（PDF→PNG） | `setup.sh` が `brew install poppler` |
| LibreOffice (`soffice`) | 必須 | xlsx の再計算検証、スライドの PDF レンダリング | `setup.sh` が `brew install --cask libreoffice` |
...
`setup.sh` は GUI アプリを自動インストールしません。Microsoft Office が必要な
場合は、Homebrew Cask で次を実行します。

```sh
brew install --cask microsoft-office
```

インストール後、Word / Excel / PowerPoint を起動し、必要に応じて Microsoft 365
アカウントでサインインします。Homebrew Cask はアプリ本体の導入だけを行い、
ライセンス認証やアカウント設定は行いません。
...
`~/.claude/settings.json` は通知以外の設定も持つため、
