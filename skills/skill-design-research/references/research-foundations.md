...
| **Keep always-loaded context tight.** | Anthropic's context-engineering guidance warns against bloated tool sets and laundry-list prompts, while recent prompt studies show that too many simultaneous constraints degrade instruction following. | Put only always-needed rules in global instructions or `SKILL.md`; move optional depth to references, tools, tests, or runtime retrieval. |
| **Default to the simplest agent architecture that passes evals.** | OpenAI's evaluation guidance warns that multi-agent setups add complexity and should be justified by evaluation needs, while Anthropic's agent guidance starts from simple workflow patterns before heavier orchestration. | Do not split one workflow into multiple agents, reviewers, or evaluators unless user intent, repeated traces, or eval evidence show the simpler architecture is insufficient. |
...
| **Prefer external-evidence-first review over free-form reflection.** | Kamoi et al. synthesize self-correction limits, and RefineBench shows that untargeted self-refinement remains weak even for frontier models. | When designing instructions or skills, anchor self-review to tests, traces, renders, readbacks, diffs, or other external signals whenever they exist. |
...
| [Anthropic, "Effective context engineering for AI agents"](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents) | Official engineering guidance | Context-budget management, canonical examples over edge-case lists, just-in-time retrieval |
| [Anthropic, "Writing effective tools for agents"](https://www.anthropic.com/engineering/writing-tools-for-agents) | Official engineering guidance | Tool naming, tool boundaries, token-efficient responses, and ergonomics that beat prompt bloat |
...
| [Mei et al., "A Survey of Context Engineering for LLMs"](https://arxiv.org/abs/2507.13334) | Survey preprint | Prompting as one part of a broader context surface; context engineering tradeoffs |
| [Harada et al., "When Instructions Multiply"](https://aclanthology.org/2025.findings-emnlp.896/) | Peer-reviewed paper | Instruction-following degradation under stacked constraints, including code tasks |
| [Qi et al., "AGENTIF"](https://openreview.net/forum?id=FLiMxTkIeu) | Benchmark / conference spotlight | Long, tool-heavy agentic prompt difficulty and eval framing |
...
| [Kamoi et al., "When Can LLMs Actually Correct Their Own Mistakes?"](https://arxiv.org/abs/2406.01297) | Peer-reviewed survey | Limits of intrinsic self-correction and value of external feedback |
| [Lee et al., "RefineBench"](https://openreview.net/forum?id=Ycred6ETQR) | Workshop benchmark | Practical limits of self-refinement without targeted feedback |
| [Xu et al., "Tool learning with language models: a survey"](https://link.springer.com/article/10.1007/s41019-025-00296-9) | Peer-reviewed survey | Tool-selection, argument, chaining, and robustness evaluation dimensions |
