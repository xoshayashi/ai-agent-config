...
def _apply_tokens(tokens: dict) -> None:
    """トークン(基本デザイン)から派生する全モジュール定数を一度に組み直す。

    テンプレートを切り替えると色・型スケール・余白が変わる — が、これらから import 時に
    導いた定数(C/TS/LAY/OPT/…)を後から個別に代入し直すと必ず1つ取りこぼす。派生はすべて
    この1関数に集約し、import 時に base で、build() で解決済みトークンで呼ぶ。行間・光学較正・
    フォント・スライド寸法・行分割はテンプレート不変(resolve_tokens が保証)。"""
    global TOKENS, C, TS, LAY, LINE_BREAK, CH, FONTS, OPT
    global SLIDE_W, SLIDE_H, MX, CONTENT_W, FOOT_Y
    global DISPLAY_WRAP_MAX_CHARS, DISPLAY_WRAP_MAX_LINES, LABEL_MAX_CHARS
    TOKENS = tokens
    ...
    DISPLAY_WRAP_MAX_CHARS = LINE_BREAK["max_display_chars_ja"]   # これを超えたら本文 — 自然に埋める
...
def slot_lines(pattern: str, slot: str, text: str) -> int:
    ...
    cfg = next(c for c in header_slots(pattern, TOKENS) if c["slot"] == slot)
...
def add_act_chart(slide, x, y, w, h, cspec: dict):
    ...
    if ctype != "donut":
        ...
        if cspec.get("y_min") is not None:
            val_ax.minimum_scale = _axis_bound(float(cspec["y_min"]))
...
def p_competitive_landscape(slide, spec, deck):
    ...
    # 軸ラベルの枠は文字ぶんの大きさで組む(固定幅だと語が割れる)。プロットのラベルはこの枠を
    # 避けて置かれる — 軸の語の上に重ねない
    ax_pt = TS["chart_axis"]
    ax_h = ax_pt / 72.0 * leading(ax_pt) + 0.02
    placed: list[tuple[float, float, float, float]] = []

    right_edge = (grid(8, 4)[0] - 0.12) if spec.get("notes") else (MX + CONTENT_W)

    def _axis_label(text, x, y, align=PP_ALIGN.LEFT, max_w=None):
        """軸ラベルの枠は文字ぶん。置き場が足りないときだけ、その幅で折り返す(語は割れない)。"""
        if not text:
            return
        w_ = _label_w(text, ax_pt, 400)
        if max_w:
            w_ = min(w_, max_w)
        lines = _text_lines(text, w_, ax_pt, 400)
        h_ = lines * ax_pt / 72.0 * leading(ax_pt) + 0.02
        lx = x - w_ if align == PP_ALIGN.RIGHT else (x - w_ / 2 if align == PP_ALIGN.CENTER else x)
        add_text(slide, lx, y - (h_ - ax_h) / 2, w_, h_, [[(text, ax_pt, 400, C["ink_faint"])]],
                 align=align)
        placed.append((lx, y - (h_ - ax_h) / 2, lx + w_, y + h_ - (h_ - ax_h) / 2))

    _axis_label(ax.get("low", ""), cx - 0.12, cy + size / 2 - ax_h / 2, PP_ALIGN.RIGHT)
    _axis_label(ax.get("high", ""), cx + cw_ + 0.10, cy + size / 2 - ax_h / 2,
...
def p_guidance_progress(slide, spec, deck):
    ...
    if not bars:
        ...
        # 枠は文字ぶんの高さ(大きな数字の行ボックスは高い)。下に置く行はその実高から積む
        pct_h = drawn_line_h(pct_pt) + 0.02
        add_text(slide, x + 0.34, py + 0.80, w * 0.42, pct_h,
                 [[(pct_label, pct_pt, 700, C["primary_deep"])]],
                 anchor=MSO_ANCHOR.MIDDLE, wrap=False)
        supporting = cur.get("actual_display", "")
        if supporting:
            add_text(slide, x + 0.38, py + 0.80 + pct_h + 0.06, w * 0.46,
                     drawn_line_h(TS["body"]) + 0.02,
...
def main() -> None:
    from deck_text import list_templates
    ap = argparse.ArgumentParser()
    ap.add_argument("spec", type=Path, nargs="?")
    ap.add_argument("-o", "--out", type=Path, default=None)
    ap.add_argument("--templates", action="store_true",
                    help="list the available design templates (meta.template) and exit")
    args = ap.parse_args()
    if args.templates:
