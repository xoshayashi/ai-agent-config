...
INK_KIND_PREFIX = "ink:"   # 図形名に刻む種別タグ(検証側が同じモデルで測るための宣言)
BOX_SLACK_IN = 0.02        # 行ボックスに対する最小の逃げ(丸めでインクが切れないため)


def _parts_lines(parts, width_in: float) -> int:
    """そのブロックが実際に何行で描かれるか。行数はブロックの持ち物であって呼び出し側が
    渡す前提ではない — 渡し忘れると1行として高さを積み、下の要素へ食い込む(値+単位のように
    ランが複数ある段落は折返しの対象外なので、合計幅から行数を出す)。"""
    if len(parts) == 1:
...
def stack_optical(slide, x, y, w, h, blocks, *, align=PP_ALIGN.LEFT):
    ...
    heights = [_ink_h(b, w) for b in blocks]
    ...
    for b, ih, gap in zip(blocks, heights, gaps):
        ...
        # 箱はインクを囲む「位置決めの枠」であって、文字の器ではない — 行ボックス(和文の
        # ディセンダ余白、数字の上寄り)まで箱に含めると、その余白が隣のブロックへ食い込み、
        # 描画は正しいのに枠だけが重なる(PowerPoint で編集すると重なった枠を掴む)。
        # MIDDLE 寄せなら文字は箱の中心に揃うので、箱が行ボックスより低くてもインクは動かない
        box_h = ih + 2 * BOX_SLACK_IN
        ink_center = cy + ih / 2
        box_center = ink_center - b["size"] / 72.0 * offset[b.get("kind", "text")]
        tb = add_text(slide, x, box_center - box_h / 2, w, box_h, [b["parts"]],
                      align=b.get("align", align), anchor=MSO_ANCHOR.MIDDLE,
                      line_spacing=b.get("line_spacing", 1.2))
        # インク種別は描く側が宣言する。検証側が文字列から推測すると(数字比率など)、
        # 「黒字化」のような数値ブロックを text として測り、ビルダーと違うインクを見る
        tb.name = f"{INK_KIND_PREFIX}{b.get('kind', 'text')}"
        cy += ih
    return total
