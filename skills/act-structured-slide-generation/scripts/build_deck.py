...
from deck_text import (footer_text, header_slots, hw, ja_len as _ja_len, one_line_chars,
                       wrap_display)
...
def display_wrap_text(text: str, w_in: float, size_pt: float) -> str:
    """表示テキスト(ラベル・見出し・箇条書き・結論句)の改行位置を文節の切れ目へ寄せる。
    すでに手で改行してあるもの、長すぎる本文は素通し(レンダラの自然折返しに任せる)。
    ...
    if not isinstance(text, str) or "\n" in text or _ja_len(text) > DISPLAY_WRAP_MAX_CHARS:
        return text
    cap = max(1.0, one_line_chars(w_in, size_pt))
    natural = math.ceil(_ja_len(text) / cap)
    return wrap_display(text, w_in, size_pt, min(DISPLAY_WRAP_MAX_LINES, natural + 1))
...
def add_text(slide, x, y, w, h, runs, *, align=PP_ALIGN.LEFT, anchor=MSO_ANCHOR.TOP,
    ...
    if display_wrap and wrap:
        runs = [
            [(display_wrap_text(para[0][0], w, para[0][1]), *para[0][1:])] if len(para) == 1
            else para
            for para in runs
        ]
...
def _ink_h(b) -> float:
    ...
    n = max(1, b.get("lines", 1))
    line_h = b["size"] / 72.0 * b.get("line_spacing", 1.2)
    return (n - 1) * line_h + b["size"] / 72.0 * OPT["ink_ratio"][b.get("kind", "text")]


def stack_optical_height(blocks) -> float:
    """縦積みが必要とするインク総高(in)。器の高さを決める側と描く側が同じ式を見る。"""
    return sum(_ink_h(b) for b in blocks) + sum(b.get("gap_before", 0.0) for b in blocks[1:])


def stack_optical(slide, x, y, w, h, blocks, *, align=PP_ALIGN.LEFT):
    """カードの矩形 (x, y, w, h) に blocks を縦積みする。インクの隙間で組む。

    blocks: [{parts, size, kind, gap_before, lines, line_spacing, align}] のリスト
      parts        : add_text と同じ (text, size, weight, color) のリスト(1段落ぶん)
      size         : その段落の級数(インク高の基準)
      kind         : "text"(和文中心) か "numeral"(大きな数字) — インク高と中心オフセットが違う
      gap_before   : 直前のブロックのインクとの隙間(in)。先頭は無視
      lines        : 折返し行数(既定1)
    返り値: 実際に使ったインク総高(in)
    """
    offset = OPT["ink_center_offset_em"]
    heights = [_ink_h(b) for b in blocks]
    gaps = [0.0] + [b.get("gap_before", 0.0) for b in blocks[1:]]
    total = sum(heights) + sum(gaps)
    cy = y + max(0.0, (h - total) / 2)          # 上下インセットは対称(残りを均等に)
    for b, ih, gap in zip(blocks, heights, gaps):
        cy += gap
        # 箱を縦中央寄せで置き、箱中心をずらしてインク中心を狙った位置に合わせる
        box_h = ih + b["size"] / 72.0 * 0.8      # インクが箱からはみ出さない余裕
        ink_center = cy + ih / 2
        box_center = ink_center - b["size"] / 72.0 * offset[b.get("kind", "text")]
        add_text(slide, x, box_center - box_h / 2, w, box_h, [b["parts"]],
                 align=b.get("align", align), anchor=MSO_ANCHOR.MIDDLE,
...
def add_bullets(slide, x, y, w, h, items, size, color, *, line_spacing=1.3,
    ...
    for i, txt in enumerate(items):
        txt = display_wrap_text(txt, text_w, size)
        p = tf.paragraphs[0] if i == 0 else tf.add_paragraph()
...
def p_executive_summary(slide, spec, deck):
    ...
    def _content_h(p):
        hl = _text_lines(p.get("heading", ""), tw, TS["section_heading"])
        bl = _text_lines(p.get("body", ""), tw, TS["body"])
    ...
    def card_blocks(k):
        ...
        blocks = [
            {"parts": [(k.get("label", ""), TS["kpi_label"], 600, C["ink_subtle"])],
             ...
             "lines": _text_lines(k.get("label", ""), text_w, TS["kpi_label"])},
            {"parts": vparts, "size": vsize, "kind": "numeral",
             "gap_before": gap["value_meta"]},
        ]
        if k.get("delta"):
            ...
            blocks.append({
                ...
                "lines": _text_lines(dtext, text_w, TS["kpi_sub"])})
        if k.get("note"):
            blocks.append({
                "parts": [(k["note"], TS["kpi_sub"], 400, C["ink_faint"])],
                "size": TS["kpi_sub"], "kind": "text", "gap_before": gap["value_meta"],
                "lines": _text_lines(k["note"], text_w, TS["kpi_sub"]), "line_spacing": 1.18})
        return blocks

    stacks = [card_blocks(k) for k in kpis]
    inner_hs = [stack_optical_height(b) for b in stacks]
...
def p_chart_insight(slide, spec, deck):
    ...
    if layout == "chart_full" or not takeaways:
        ...
        if akind:
            ...
            add_unit_note(slide, cx, y0 + 0.02, chart.get("unit"))
    ...
    if rows:
        ...
        for row in rows:
            ...
            for c_i in range(n):
                ...
                if cell:
                    ln = max(ln, _text_lines(str(cell), cell_w, TS["body_small"]))
