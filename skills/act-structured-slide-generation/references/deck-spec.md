...
that carry the hero's unit (native series, or a combo's `bar` / `line` — a `%` line never proves a
`億円` hero), or to be the last-category total of a stacked chart, or to be a same-unit
`derivation` whose operands are chart values or `chart…` paths (a CAGR in `%` derived from a
`億円` series is the normal case) — the chart proves the hero, never the other way round. A
