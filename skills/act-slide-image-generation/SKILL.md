...
description: "Create ACT-style 16:9 strategy slide image prompts and actual gpt-image-2 bitmap slide images from briefs, long text, notes, screenshots, rendered slide images, or deck outlines. Use as the primary skill for ACT slide-structure planning, ACT bitmap/image-prompt planning, Codex built-in image generation, critique, iterative pre-Google Slides image self-review, repair, and image-only Google Slides roll-up with speaker notes. The ACT design system is embedded in this SKILL.md; do not load or depend on an external ACT slide pattern file. When the user asks for slide image generation, do not substitute deterministic renders, screenshots, or exports for gpt-image-2 output."
---
...
Create ACT strategy slide image specifications and final `gpt-image-2` prompts. This skill is self-contained: do not read an external ACT slide pattern file to know the design system. Long text is first converted into a deck storyline and per-slide structure; each slide then keeps one claim, one dominant structure, fixed header/footer anchors, object-level coordinates, master components, selective Insight components, ACT visual language, and speaker notes for Google Slides delivery.
...
Do not treat first generated images as complete. Before any Google Slides insertion, run a multimodal self-review on the actual generated PNGs, repair and regenerate failed slides, and repeat until the slide images are judged presentation-quality against the ACT gates below.
...
- The slide should feel like an ACT strategy document: calm, structured, slightly dense, claim-led, and designed with visible editorial judgment.
...
- deck_tone_master_lock includes slide base, typography scale, header/footer lock, Petrol usage, Honey usage, illustration style, density rhythm, whitespace/occupancy rhythm, card/table geometry, outer padding, Source baseline, and negative prompt.
...
- Surfaces: use pale solid Panel Mint cards/tables with thin rules, small radii, consistent padding, and equalized heights. Prefer a single component skeleton reused across table cells, cards, rails, and process nodes.
...
- Color: Petrol is the only strong recurring color and reads as one system per slide. Pale gray fills and Forest Charcoal carry most of the page. Honey is rare, pale, flat, and used for interpretation only.
...
- `Petrol-headed system`: table, card, phase, matrixに読み順を与えるheader band。
...
- Petrol-headed systems require equal header heights and equal color role.
...
- Left vertical line is Petrol `#008A80`, no number attached.
- H1 is Forest Charcoal `#2D332E`, `700`, `30-34pt`.
...
- Petrol and Honey are not header/footer text colors. Petrol appears in the header line or structural rules; Honey appears only in approved Insight/message surfaces.
...
- Standard exact `1672` ACT header line is `x=50 y=48 w=10 h=104`. Standard exact H1 starts at `x=88 y=34`.
...
vertical_line: x=50 y=48 w=10 h=104 color #008A80
...
- Footer/source/table-note text always uses Ink-3 `#6E756E`, `400`. Do not use Petrol, Honey, Forest Charcoal, random gray, or opacity variants for footer text.
...
| Token | Color | Use |
| --- | --- | --- |
| Slide base | `#FCFBF8` | slide background |
| Panel Mint | `#F7FBF9` | panel / neutral box background |
| Forest Charcoal | `#2D332E` | H1 / body / Insight text |
| Ink-2 | `#4D544E` | subtitle / secondary text |
| Ink-3 | `#6E756E` | Source / tertiary text |
| Petrol | `#008A80` | header line / structural accents / table header |
| Petrol deep | `#004F49` | dark Petrol surface |
| Petrol mist | `#B3DDD9` | quiet Petrol surface |
| Honey surface | `#F5E2A8` | flat pale Honey message box fill |
| Honey accent | `#C49A2C` | thin Honey left accent line only |
| Honey micro | `#ECC85A` | optional tiny cue only, never message box fill |

Petrol:

- petrol_usage_lock: Petrol is a structural color, not a general emphasis color.
- Exact primary Petrol is `#008A80`. Do not introduce extra navy, royal blue, cyan-blue, blue gradients, or opacity-made variants.
- Always use Petrol for the left header line.
- In the body, choose one active Petrol system per slide: table/card header band, process lane spine, matrix axes/quadrant highlight, structural divider/rule, step badges, or one key-number marker system.
- The header line does not count against the active body system. Body Petrol should read as one coordinated system, not separate blue decorations.
- Standard Petrol area budget is `6-12%`; dense table and chapter/closing slides may sit near the top of that range. Do not exceed `12%`.
- Do not use Petrol as H1, subtitle, body paragraph text, footer/source/table-note text, arbitrary icon color, decorative illustration fill, or message-box wallpaper.
- Petrol deep `#004F49` is reserved for rare dark surfaces or strongest header bands; if used, it replaces other strong blue areas on that slide.
- Petrol mist `#B3DDD9` is a quiet structural tint only. Text on mist stays Forest Charcoal `#2D332E`; do not make blue-on-blue text.
- If a dark Petrol bottom surface is used, quiet the table header, right rail, card headers, icons, and key numbers so only one blue area leads.

Insight surface family:

- Honey and Petrol boxes share the same geometry: radius, height, padding, text baseline, left accent line, background treatment.
- Petrol surface is for structure, trust, system logic, and quiet conclusion.
...
- Petrol surface background: `#B3DDD9`; left accent: `#008A80` or `#004F49`.
- Honey surface background: flat pale Honey `#F5E2A8`; left accent: muted Honey `#C49A2C`.
- Honey surfaces must feel quieter than Petrol surfaces. Use pale fill, Forest Charcoal text, and one thin left accent line; do not use saturated yellow as a large area.
...
- Dark Petrol bottom appears in about 1 slide for an 8-10 slide deck and 1-2 slides for a 12 slide deck.
...
- Honey message box fill must be `#F5E2A8`; accent line must be `#C49A2C`; text remains Forest Charcoal `#2D332E`.
- Honey left accent line is `4-5px` wide and full height. Do not vary its thickness, color, or side within a deck.
- If Honey is used, quiet one of the Petrol surface, right rail, or heavy heading.
...
- Icon circle uses pale Panel Mint field and Petrol stroke icon.
...
- Step badge: Petrol filled circle, white number, diameter `30-42px`.
...
- ACT visuals are editorial illustration, diagram, structure map, and line icon visuals.
...
- technical editorial line illustration: clean controlled vector linework, crisp silhouettes, intentional simplification, pale Panel Mint fills, restrained flat shading, a clear focal motif, only useful supporting details, deliberate whitespace, Petrol structural accents, optional Honey focus cue when it aids interpretation.
- For workflow/process/before-after claims, use technical storyboard grammar when helpful: three aligned line-illustration panels, Petrol arrows, lower icon strip, separate Insight surface.
- Translate rooms, workers, robots, tools, dashboards, facilities, stores, factories, and streets into clean line drawings with pale Panel Mint fills.
...
- Header band is Petrol fill with `#FCFBF8` or white text.
...
- Highlight key numbers in Petrol; body explanation stays Forest Charcoal.
- Row highlight uses Panel Mint. Honey only for decision signal.
...
- Card border is thin Petrol or thin Ink.
...
| --- | --- | --- |
| `outlined top thesis` | early thesis | `#FCFBF8` fill + Petrol border + bold centered text |
| `icon thesis band` | market definition / beachhead / why now | left icon circle + sentence |
| `outlined bottom conclusion` | readout after table/figure | `#FCFBF8` fill + Petrol border |
| `Petrol surface conclusion` | structure/trust/system readout | Petrol mist surface |
| `dark Petrol conclusion` | strong section close | dark Petrol fill + `#FCFBF8` text |
...
Petrol header systems:

- Tables, phase cards, matrix panels, and persona card groups can use Petrol header bands.
- Same hierarchy = same header band height.
- Header text is white or `#FCFBF8`, `600` or `700`, short.
- Large tables can have fixed left icon or label column.
- Multiple Petrol headers must align to column lines and read as one system.
...
| `product-stack + trust-capability-grid` | product stack / functions / trust | `full-12` | 3 | dark Petrol bottom |
...
- H1/body are Forest Charcoal.
- Petrol `#008A80` and Panel Mint `#F7FBF9` are structural.
- Honey is a decision signal.
...
- No Petrol, Honey, saturated yellow, or arbitrary gray appears in header/footer text.
- Honey max 1 per slide; Honey component uses left accent + tint background.
- Petrol and Honey surfaces share one component skeleton.
...
- Honey message boxes use `#F5E2A8` fill, `#C49A2C` left line, and `#2D332E` text consistently. Strong yellow fills are not part of the component.
...
- In `post_generation_design_balance_check`, explicitly inspect canvas occupancy versus blank zones, external padding drift, header line/H1/subtitle integrity, card/table height equalization, line-weight consistency, icon-family consistency, Petrol scatter, Honey strength, and whether illustrations look like human-designed operational diagrams rather than generated concept art.
- Score the whole image set on `deck_tone_consistency`: palette role consistency, Petrol usage consistency, Honey treatment consistency, header/footer consistency, illustration style family, icon family, density rhythm, card/table geometry, whitespace rhythm, source baseline, and first-third vs last-third tone match.
...
```text
ACT slide contract:
- use embedded ACT design system in SKILL.md
- do not load an external ACT pattern file
- 1 slide = 1 claim
- 1 slide = 1 dominant structure
...
- lock deck_header_master: left vertical line #008A80, H1 #2D332E / 700 / 30-34pt, subtitle #4D544E / 400 / 26-30pt
...
- use Petrol and Honey Insight surfaces in one surface family
...
2. Use the embedded ACT design system above. Do not load any external ACT pattern file.
...
9. Run generation readiness: built-in image generation available, prompt final, size valid, sources resolved, ACT design system reflected, header master locked, visual richness/density complete, speaker notes drafted, pilot plan chosen.
...
- Specify `image_model: gpt-image-2`, `16:9`, valid image-generation size, ACT target `1920x1080`, and `1672x941` coordinate basis.
...
Use `scripts/build_act_slide_prompt.py` to create a prompt scaffold from a rough brief:

```bash
python3 scripts/build_act_slide_prompt.py brief.md
python3 scripts/build_act_slide_prompt.py long-memo.md --mode text-structure --size 2048x1152
python3 scripts/build_act_slide_prompt.py brief.md --mode single-slide-image --archetype "market-table + layered-rail" --grid-mode "70/30"
python3 scripts/build_act_slide_prompt.py brief.md --mode repair
python3 scripts/build_act_slide_prompt.py brief.md --mode audit
```
