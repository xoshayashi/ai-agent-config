...
Freeze a `zonal_mass_plan`, `layout_tree`, `grid_plan`, `flex_plan`, and `occupancy_plan` before generation. On the 1672x941 basis, header visible marks stay within `y=66..155`; the body envelope uses `y=190..869` without a footer and `y=190..842` with a footer, and the first meaningful body region begins 48-112px below the `y=170` header clearance datum. A visible provenance footer uses `x=72 y=866 w=1528 h=48`, 9pt `footer` text, zero insets, left alignment, centered vertical anchoring, and a one- or two-line budget. Footer-absent slides pass 82-94% complete-body height and 65-80% main-container height; footer-present slides pass 78-90% complete-body height and 65-85% main-container height. Both modes pass 78-92% primary-body width, 82-94% primary-container width, 58-80% allocated-region area fill, and 18-38% meaningful-foreground fill. Measure text ink at 6-20% and meaningful-object ink at 10-30% of the visible body envelope. A single aspect-locked focal object keeps its 55-88% object-width band while the complete body passes its footer-mode bands. Compute vertical centroid from every painted body component, including components attached directly to a Grid parent and the painted message box; it passes 58-62% of the full canvas height without a footer and 55-60% with a footer. The full-width message box is an auxiliary conclusion band: include it in vertical mass, area fill, and closure, while primary-body horizontal occupancy is measured from non-message components. Footer-present layouts use the dedicated clear body envelope and therefore omit the canonical bottom message box. This middle-band contract keeps the body visually substantial while preserving distinct header, content, and footer zones.
...
Apply `chart_emphasis_lock`: mute non-focal series, place the decision-carrying value beside its mark, and label series directly when space permits. Apply `encoding_consistency_lock` so the same meaning keeps the same fill, shape, and scale across the deck. Apply `number_format_normalization_lock` before exact-copy freeze, and register a `benchmark_reference_line` whenever the decision depends on a target, prior period, or external comparator.
