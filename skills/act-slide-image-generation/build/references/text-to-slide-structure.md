...
- Apply `icon_restraint_lock`: icons are sparse wayfinding or evidence markers. Avoid icon-and-keyword-only slides, generic icon grids, icon wallpaper, repeated icons, or icons that replace clearer sentences, tables, causal chains, or labeled diagrams.
- Apply `icon_density_budget`: default to 0-2 purposeful icons per slide; allow 3 only when each icon has a distinct decision-critical role recorded in `icon_justification_gate`.
- Apply `icon_justification_gate` and `icon_location_lock`: every icon must name its reading-path, evidence, decision, or navigation job and declare its location. Decorative, redundant, generic, one-icon-per-card, noun-label replacement, and auto-added card/row/label/diagram icons are blockers.
- Apply `icon_box_compaction_lock`: icons must not set card height, enlarge message boxes, increase row height, create tall empty wells, or widen top/bottom gaps. Remove or shrink the icon before increasing box height or padding.
...
- Apply `occupancy_density_fit_lock`: body occupancy must match the selected density tier without accidental dead zones, crushed margins, overcrowding, or density created by shrinking body/card/table/data text below 20pt equivalent.
...
- Apply `ergonomic_min_text_size_lock`: body labels, card rows, table cells, data labels, annotations, and Insight text must be at least 20pt equivalent; source/footer/table-note text may be 13-15pt equivalent.
...
- Apply `header_accent_geometry_lock`, `header_line_top_rule`, and `header_left_accent_title_top_alignment_lock`: the left vertical line is the approved header-block anchor, not a short title tick. On the 1672x941 basis, use `vertical_line x=50 y=37 w=10 h=41 #008A80` inside the fixed 105px planning header zone unless a newer embedded master is supplied; on 2048x1152 output use the scaled geometry `x=61 y=45 w=12 h=50` with bottom around `y=95`. Its top must align exactly with the first visible H1 glyph/title top, not the H1 text-box y; it may not sit above the title, start below the title, float near the page top, clip outside `header_safe_area`, detach from H1/subtitle, or intrude into the body. One visible pixel above the H1 glyph top is a blocker. If it fails, repair the line x/y/h before touching H1 or body layout.
...
- Define `deck_tone_master_lock` before image generation and check generated images for whole-deck tone consistency before PPTX/PDF packaging. Freeze the fixed slide shell before slide-level layout: 2048x1152 output uses 64px left/right outer padding and 48px content-zone top/bottom padding; the 1672x941 planning basis uses 52px left/right outer padding and 39px content-zone top/bottom padding. Reuse those values, footer baseline x/y, card radius, and each text-role point size verbatim on every slide, exactly as the header master is frozen.
...
6. **Density And Split Gate**
   ...
   - Split when a slide has multiple messages, mixed structures, more than 3 major regions, too many source facts, or main content text that would fall below 20pt equivalent.
...
- `overload_controls`: one dominant structure, max three major regions, grouped labels, body/card/table/data text at 20pt equivalent or larger, no icon-driven box bloat, and no illustration detail doing the job of evidence.
...
- Main content text would need to fall below 20pt equivalent.
