...
def canonical_planning_block(
  ...
  deck_tone_master_lock: [slide base, typography scale, header/footer, Deep Blue use, Honey use, illustration style, icon family, density rhythm, whitespace/occupancy rhythm, card/table geometry, outer padding, invisible source alignment baseline, negative prompt]; freeze one exact value (not a range) for outer_padding, footer baseline x/y, card radius, message-box height, and each text-role point size at deck-master time and reuse it verbatim on every slide, exactly as the header master is frozen
