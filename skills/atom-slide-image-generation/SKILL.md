...
Favor message-led structure, useful density, compact message boxes, exact visible text, meaningful graphics, thumbnail legibility, and clear reading path. During slide-structure planning, apply `impact_clarity_density_gate`, `message_sharpness_lock`, and `evidence_compression_ladder` so each slide has a sharp takeaway, self-explanatory logic, simple dominant structure, and enough decision-relevant proof before image prompting. Only `exact_text` may render visibly; workflow/audit metadata and speaker notes stay non-rendered.
