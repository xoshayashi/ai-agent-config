...
description: "Use when generating, reviewing, repairing, or PPTX-packaging ATOM 16:9 strategy slide images from briefs, text, notes, screenshots, or deck outlines with gpt-image-2 and review-gated PNG/PPTX output."
---

# ATOM Slide Image Generation

Use `scripts/build_atom_slide_prompt.py` for deck or single-slide prompt scaffolds; it loads full runtime rules from `build/references`. Use `scripts/package_slide_images_to_pptx.py` only after approved PNGs, notes, and review manifest exist.
...
Design: Noto Sans JP, 12-column 1672x941, Deep Blue `#0B2F5B`, Charcoal Ink `#2D332E`, light gray `#DDE3EA`, quiet Honey `#F7EECF/#C49A2C`, and one flat 2D editorial illustration style across people, UI, icons, linework, fills, crop, and face/body detail.
