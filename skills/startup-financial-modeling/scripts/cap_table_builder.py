"""
cap_table_builder.py — Cap Table state machine + 08_CapTable シート生成
...
def run_round_state_machine(inp: CapTableInput) -> RoundResult:
    ...
    for s in cascaded_safes:
        ...
        if s.mfn and s.name in (cascaded_safes[i].name for i in range(len(cascaded_safes))):
            sc.mfn_applied = bool(mfn_changed)
...
def build_cap_table_sheet(
    ...
    *,
    ...
    """Generate / replace the 08_CapTable sheet with full trace.
    ...
    if "08_CapTable" in wb.sheetnames:
        ...
        for row in ws.iter_rows():
            for cell in row:
                ...
                cell.fill = ib.PatternFill(fill_type=None) if hasattr(ib, "PatternFill") else cell.fill
    else:
        ws = wb.create_sheet("08_CapTable")
    ...
    for s in inp.safes:
        rows.append((
            ...
            s.type,
            float("nan"),  # phantom
            f"{s.principal_money_m:,.1f}{money_unit}, cap {s.cap_money_m or 'n/a'}, disc {s.discount:.0%}{', MFN' if s.mfn else ''}",
        ))
    ...
    # Pre-FDSO total
    # Phase 6 Wave 3: register Cap_FDSO_Pre as workbook-scoped named range
    # (canonical: references/_named_ranges.md §2.8, single-cell)
    r += 1
    ...
    ib.apply_grand_total(
        ws.cell(row=r, column=4), fmt=ib.FMT_SHARES,
        named_range="Cap_FDSO_Pre",
    )
    ...
    if round_result is not None:
        ...
        # Phase 6 Wave 3: Cap_PoolTopUp single-cell named range
        ib.apply_formula(
            ws.cell(row=r, column=4), fmt=ib.FMT_SHARES,
            named_range="Cap_PoolTopUp",
        )
        r += 1
        ...
        # Phase 6 Wave 3: Cap_NewRoundShares + Cap_PPS_PostMoney single-cell named ranges
        ib.apply_formula(
            ws.cell(row=r, column=4), fmt=ib.FMT_SHARES,
            named_range="Cap_NewRoundShares",
        )
        ws.cell(row=r, column=5).value = rr.new_round_price
        ib.apply_formula(
            ws.cell(row=r, column=5), fmt=ib.FMT_PER_SHARE,
            named_range="Cap_PPS_PostMoney",
        )
        r += 1
        ...
        # Phase 6 Wave 3: Cap_FDSO single-cell named range (canonical §2.8 master FDSO)
        ib.apply_grand_total(
            ws.cell(row=r, column=4), fmt=ib.FMT_SHARES,
            named_range="Cap_FDSO",
        )
        r += 1
...
def build_cap_table_for_workbook(
    ...
    *,
    ...
    """(Re-)build 08_CapTable sheet on the given workbook.
...
def get_default_input() -> CapTableInput:
    """Demo: evals.json case 2 (jkiss_to_series_a_conversion).

    Two J-KISS 2.0 notes:
      - ¥80M @ cap ¥1.0B post, 20% disc, 2024-06
      - ¥40M @ cap ¥1.5B post, 15% disc, 2025-01, MFN
    Founder 8.0M, common pool 1.5M (granted 0.4M, available 1.1M).
    Series A: pre ¥3.2B, raise ¥800M, ESOP target 12% post-money.
    """
...
def _cli() -> None:
    parser = argparse.ArgumentParser(
        description="Cap Table Builder — 08_CapTable sheet generator + State Machine.",
    )
