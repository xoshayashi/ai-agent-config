...
# Benchmarks register starts at C (col 3) so column B can hold the canonical
# 2.14-wide indent spacer (see _layout_canonical.md).
BENCHMARK_REGISTER_START_COL = 3
...
def _autosize_default_layout_columns(wb: Workbook) -> None:
    """Content-drive label / source / note column widths on default-layout
    sheets.

    Runs after all data is written so the measurement reflects the final
    cell content. Per spec.md §2 C1 the period and unit columns stay at
    their canonical fixed widths — they carry numbers under a uniform
    visual rhythm, not free-form text. Custom-layout sheets (Driver Tree,
    Cap Table, Exit Waterfall, Benchmarks) keep the widths their builders
    chose. Role-min floors match the canonical starting widths, so any
    sheet whose labels already fit at the floor stays exactly there — no
    surprise narrowing.
    """
    for ws in wb.worksheets:
        ...
        note_col = _start_period_col() + len(facts.period_labels)
        if any(
            ws.cell(row=r, column=note_col).value not in (None, "")
            for r in range(1, ws.max_row + 1)
        ):
            role_columns[note_col] = "note"
        ib.autosize_role_columns(ws, role_columns)
...
def _setup_sheet(
    ...
    ib.apply_indent_column_widths(
        ws, range(LAYOUT.first_hierarchy_col, LAYOUT.label_col)
    )
...
def _comment_col(facts: SourceFacts) -> int:
    return _start_period_col() + len(facts.period_labels)
...
def _write_period_header(ws: Worksheet, facts: SourceFacts, row: int = 5) -> None:
    for col in _period_cols(facts):
        ws.column_dimensions[get_column_letter(col)].width = LAYOUT.period_width
    comment_col = _comment_col(facts)
    ws._startup_note_col = comment_col
    ws.column_dimensions[get_column_letter(comment_col)].width = LAYOUT.note_width
    ...
    headers = [
        (LAYOUT.first_hierarchy_col, ""),
        (LAYOUT.label_col, "Line item"),
        (LAYOUT.source_col, "Source / driver"),
        (LAYOUT.unit_col, "Unit"),
        (comment_col, "Comment"),
    ]
    for col, label in headers:
        ...
        c.alignment = Alignment(
            horizontal="left" if col in (LAYOUT.first_hierarchy_col, LAYOUT.label_col, LAYOUT.source_col, comment_col) else "right",
            vertical="center",
            wrap_text=False,
        )
...
def _highlight_row(ws: Worksheet, row: int, last_col: int | None = None) -> None:
    ...
    if isinstance(note_col, int):
        end_col = max(end_col, note_col)
    ib.apply_semantic_fill_span(
        ws,
        row,
        LAYOUT.first_hierarchy_col,
...
def _write_values(
    ...
    *,
    ...
    if bold:
        # Bold font is per-cell — skipping empty middle cells (source, unit)
        # is correct, an empty cell does not need bold weight.
        for col in (LAYOUT.label_col, *period_cols):
            cell = ws.cell(row=row, column=col)
            if cell.value is None:
                continue
            font = copy(cell.font)
            font.bold = True
            cell.font = font
        # Border is span-wide — it must NOT skip empty middle cells, or the
        # subtotal underline reads as ragged. The span is detected from
        # the worksheet's own semantic header fill via
        # ib.apply_semantic_border_span (Task 1.2), with the indent gutter
        # held back through border_start_col = _row_rule_start_col(ws).
        ib.apply_semantic_border_span(
            ws,
            row,
            top=ib.THIN_LINE,
...
def _build_guide(wb: Workbook, facts: SourceFacts) -> None:
    ...
    # Canonical layout: B = indent (2.14), C = label (30), D = explanation (128).
    _set_column_widths(ws, {3: 30, 4: 128})
    rows = [
    ...
    for idx, (label, text) in enumerate(rows, start=7):
        ws.cell(idx, 3, label)
        ws.cell(idx, 4, text)
        ib.apply_label(ws.cell(idx, 3), bold=True)
        ib.apply_comment(ws.cell(idx, 4), wrap_text=False)
...
def _build_assumptions(wb: Workbook, facts: SourceFacts) -> None:
    ...
    # Task 2.5 (F): the one-time / onboarding revenue per new unit is a
    # driver. Unit-sale plans book the full sale price; a stated per-unit fee
    # is an input row; a stated onboarding_months lands in the formula; the
    # 3-month default applies only when nothing is stated and is labeled a
    # placeholder rather than presented as a sourced fact.
    # Statedness, not positivity: a stated schedule (even with zeros) is an
    # authoritative input row; an empty list means "not stated".
    one_time_stated = bool(facts.one_time_revenue_per_unit_yen)
    onboarding = list(facts.onboarding_months or []) or [3.0] * len(cols)
    if facts.revenue_mode == "unit_sale":
        _write_values(ws, 19, "One-time revenue / new unit", "JPY", [f"={get_column_letter(c)}11" for c in cols], source="one-time sale price", kind="formula", fmt=ib.FMT_JPY_YEN)
    elif one_time_stated:
        _write_values(ws, 19, "One-time revenue / new unit", "JPY", list(facts.one_time_revenue_per_unit_yen), source="stated one-time / onboarding fee", kind="input", fmt=ib.FMT_JPY_YEN)
    elif getattr(facts, "onboarding_pinned", False):
        _write_values(ws, 19, "One-time revenue / new unit", "JPY", [f"={get_column_letter(c)}11*{onboarding[idx]:g}" for idx, c in enumerate(cols)], source="stated onboarding months", kind="formula", fmt=ib.FMT_JPY_YEN)
    else:
        _write_values(ws, 19, "One-time revenue / new unit", "JPY", [f"={get_column_letter(c)}11*3" for c in cols], source="placeholder: 3-month onboarding default", kind="formula", fmt=ib.FMT_JPY_YEN)
...
def _build_driver_tree(wb: Workbook, facts: SourceFacts) -> None:
    ...
    # Canonical layout: B = indent (2.14); data table starts at C.
    _set_column_widths(ws, {3: 22, 4: 44, 5: 30, 6: 42, 7: 22})
    headers = ["Layer", "Driver", "Workbook owner", "Decision relevance", "Source status"]
    for c, header in enumerate(headers, start=3):
...
def _build_cost(wb: Workbook, facts: SourceFacts) -> None:
    ...
    # Task 3.4 (H) — governor transparency: the COGS mix is profile-shaped
    # but its level is calibrated, and the reader must know that.
    governor_note = ws.cell(
        row=19,
        column=LAYOUT.label_col,
        value=(
            "Note: variable COGS, delivery, cloud, and support are governor-"
            "rescaled by one per-period factor so total COGS lands on the "
            "target gross margin — cost mix is profile-shaped, cost level is "
            "calibrated."
        ),
    )
...
def _build_pl(wb: Workbook, facts: SourceFacts) -> None:
    ...
    # D&A: straight-line on the accumulated asset base with the half-year
    # convention (Task 3.3 G) — assets acquired in the period contribute only
    # half a period's charge; the cap keeps accumulated depreciation at or
    # below gross PP&E. Self-contained on the P&L row plus cumulative Cost
    # Build CapEx, so it survives focused-bundle pruning.
    ...
    for idx, col in enumerate(cols):
        ...
        if idx == 0:
            base = f"0.5*'Cost Build'!{c}16"
            annual = f"({base})*12/'Assumptions'!{c}39"
            da_formulas.append(f"=MIN({annual},{cum_capex})")
        else:
            prev = get_column_letter(cols[idx - 1])
            base = f"SUM('Cost Build'!${first}$16:{prev}$16)+0.5*'Cost Build'!{c}16"
            annual = f"({base})*12/'Assumptions'!{c}39"
...
def _build_cf(wb: Workbook, facts: SourceFacts) -> None:
    ...
    rows = [
        ...
        (20, "Debt financing", "JPY", [f"='Capital Stack'!{get_column_letter(c)}10+'Financing'!{get_column_letter(c)}8+'Financing'!{get_column_letter(c)}11-'Capital Stack'!{get_column_letter(c)}11" for c in cols]),
        (21, "Grants / subsidies", "JPY", [f"='Financing'!{get_column_letter(c)}7" for c in cols]),
...
def _build_capital_stack(wb: Workbook, facts: SourceFacts) -> None:
    ...
    amortization = list(facts.debt_amortization_yen or []) + [0] * periods
    amortization = amortization[:periods]
    debt_balance = []
    for idx, col in enumerate(cols):
        c = get_column_letter(col)
        # Task 3.2 (D): the balance is cumulative draws (debt + converts +
        # lease) net of the contractual amortization row below it.
        debt_inflow = f"{c}10+'Financing'!{c}8+'Financing'!{c}11-{c}11"
        if idx == 0:
    ...
    rows = [
        ...
        (8, "Debt balance", "JPY", debt_balance, "drawn − repaid roll-forward", "formula", ib.FMT_MONEY),
        ...
        (11, "Debt amortization", "JPY", amortization, "contractual repayment", "input", ib.FMT_MONEY),
...
def _build_financing(wb: Workbook, facts: SourceFacts) -> None:
    ...
    rows = [
        ...
        (13, "Founder / investor secondary", "JPY", facts.secondary_yen, "company-funded liquidity use", "input", ib.FMT_MONEY),
        (15, "Financing cash inflow", "JPY", [f"=SUM({get_column_letter(c)}7:{get_column_letter(c)}11)-{get_column_letter(c)}13-'Capital Stack'!{get_column_letter(c)}11" for c in cols], "cash inflow excl. customer advances", "formula", ib.FMT_MONEY),
...
def _build_exit_waterfall(wb: Workbook, facts: SourceFacts) -> None:
    ...
    # Canonical layout: B = indent (2.14); waterfall table starts at C
    # (Case, Exit EV, Net debt, Txn costs, Equity value, Pref floor, Common pool,
    #  New investor ownership, Investor proceeds, MOIC, Founder proceeds, Walk-away signal).
    _set_column_widths(ws, {3: 18, 4: 16, 5: 16, 6: 16, 7: 16, 8: 16, 9: 16, 10: 16, 11: 18, 12: 12, 13: 18, 14: 22})
    headers = [
    ...
    cases = [
        ...
        ("Base", f"='Valuation'!{_final_period_col(facts)}27"),
        ("Upside", f"='Scenarios'!{get_column_letter(_start_period_col() + 2)}18"),
    ]
    for row, (label, exit_ev) in enumerate(cases, start=6):
        # Column map (after canonical-layout shift): C=Case, D=Exit EV, E=Net debt,
        # F=Txn costs, G=Equity value, H=Pref floor, I=Common pool,
        # J=New investor ownership, K=Investor proceeds, L=MOIC, M=Founder proceeds,
        # N=Walk-away signal.
        ws.cell(row, 3, label)
        ws.cell(row, 4, exit_ev)
        ws.cell(row, 5, f"='Capital Stack'!{_final_period_col(facts)}8-'Capital Stack'!{_final_period_col(facts)}12")
        ws.cell(row, 6, f"=MAX(0,D{row}*3%)")
        ws.cell(row, 7, f"=MAX(0,D{row}-E{row}-F{row})")
        ws.cell(row, 8, f"=MAX('Capital Stack'!{_final_period_col(facts)}7,0)")
        ws.cell(row, 9, f"=MAX(0,G{row}-H{row})")
        ws.cell(row, 10, f"='Ownership'!{_final_period_col(facts)}9")
        ws.cell(row, 11, f"=MIN(G{row},H{row})+I{row}*J{row}")
        ws.cell(row, 12, f"=IF('Valuation'!{_final_period_col(facts)}32=\"-\",\"-\",K{row}/MAX(1,'Valuation'!{_final_period_col(facts)}30))")
        ws.cell(row, 13, f"=I{row}*'Ownership'!{_final_period_col(facts)}7")
        ws.cell(row, 14, f"=IF(OR(L{row}=\"-\",L{row}<2),\"reprice / reject\",IF(M{row}<K{row},\"terms protect investor\",\"committee case\"))")
        for col in range(3, 15):
            cell = ws.cell(row, col)
            if col == 3:
                ib.apply_label(cell, bold=col == 3)
                continue
            fmt = "General" if col == 14 else ib.FMT_PERCENT if col == 10 else ib.FMT_MULTIPLE if col == 12 else _money_format(facts)
            _apply_value_style(cell, fmt)
        if label == "Base":
            _highlight_row(ws, row, 14)
...
def _build_valuation(wb: Workbook, facts: SourceFacts) -> None:
    ...
    _write_values(ws, 24, "Financing-implied EV", "JPY", [f"='Capital Stack'!{get_column_letter(c)}15" for c in cols], kind="formula", fmt=ib.FMT_MONEY)
    benchmark_freshness_cell = f"'Benchmarks'!${get_column_letter(BENCHMARK_FRESHNESS_COL)}${BENCHMARK_VALUATION_SUPPORT_ROW}"
    _write_values(ws, 25, "Supportability score", "x", [f"=MIN(1.5,MAX(0.5,0.35+IF({get_column_letter(c)}16>0,0.25,0)+IF({get_column_letter(c)}22>0,0.20,0)+IF({get_column_letter(c)}23>0,0.20,0)+IF({get_column_letter(c)}24>0,0.20,0)+IF({benchmark_freshness_cell}=\"current\",0.20,0)))" for c in cols], kind="formula", fmt=ib.FMT_MULTIPLE)
    positive_count = lambda c: f"(({c}19>0)+({c}22>0)+({c}23>0)+({c}24>0))"
    positive_sum = lambda c: f"SUM({c}19,{c}22,{c}23,{c}24)"
    positive_min = lambda c: f"MIN(IF({c}19>0,{c}19,1E+99),IF({c}22>0,{c}22,1E+99),IF({c}23>0,{c}23,1E+99),IF({c}24>0,{c}24,1E+99))"
    positive_max = lambda c: f"MAX({c}19,{c}22,{c}23,{c}24)"
    positive_avg = lambda c: f"{positive_sum(c)}/MAX(1,{positive_count(c)})"
    _write_values(ws, 26, "Selected EV low", "JPY", [f"=IF({positive_max(get_column_letter(c))}=0,0,{positive_min(get_column_letter(c))}*IF({get_column_letter(c)}25<0.9,0.75,0.9))" for c in cols], kind="formula", fmt=ib.FMT_MONEY)
    _write_values(ws, 27, "Selected EV midpoint", "JPY", [f"=IF({positive_max(get_column_letter(c))}=0,0,IF({get_column_letter(c)}25<0.9,{positive_avg(get_column_letter(c))}*0.85,{positive_avg(get_column_letter(c))}))" for c in cols], kind="formula", fmt=ib.FMT_MONEY, bold=True)
    _write_values(ws, 28, "Selected EV high", "JPY", [f"={positive_max(get_column_letter(c))}*IF({get_column_letter(c)}25>=1,1.1,1.0)" for c in cols], kind="formula", fmt=ib.FMT_MONEY)
    _highlight_row(ws, 27, _start_period_col() + len(cols) - 1)
    start_col = LAYOUT.label_col
    ...
    for row, values in enumerate(method_rows, start=37):
        for col, value in enumerate(values, start=start_col):
            ws.cell(row, col, value)
            ib.apply_comment(ws.cell(row, col), wrap_text=False)
    _section(ws, 29, "Investor return")
    _write_values(ws, 30, "Equity invested", "JPY", [f"='Capital Stack'!{get_column_letter(c)}7" for c in cols], kind="formula", fmt=ib.FMT_MONEY)
    _write_values(ws, 31, "New investor ownership", "%", [f"='Capital Stack'!{get_column_letter(c)}16" for c in cols], kind="formula", fmt=ib.FMT_PERCENT)
    _write_values(ws, 32, "MOIC at selected EV", "x", [f"=IF({get_column_letter(c)}30=0,\"-\",{get_column_letter(c)}27*{get_column_letter(c)}31/{get_column_letter(c)}30)" for c in cols], kind="formula", fmt=ib.FMT_MULTIPLE)
    _write_values(ws, 33, "Illustrative IRR", "%", [f"=IF({get_column_letter(c)}32=\"-\",\"-\",{get_column_letter(c)}32^(1/{max(idx, 1)})-1)" for idx, c in enumerate(cols, start=1)], kind="formula", fmt=ib.FMT_PERCENT)
    _highlight_row(ws, 32, _start_period_col() + len(cols) - 1)
    ...
    gate_rows = [
        ...
        ("Supportability score", ">= 0.9x", f"see {final_col}25", "decide whether midpoint is usable", "haircut selected EV or use downside"),
        ("Return hurdle", ">= 2.0x MOIC", f"see {final_col}32", "decide price / ownership acceptability", "reprice, resize, or reject"),
...
def _build_benchmarks(wb: Workbook, facts: SourceFacts) -> None:
    ...
    # Canonical layout: B = indent (2.14); register starts at C (matches BENCHMARK_REGISTER_START_COL).
    _set_column_widths(ws, {3: 14, 4: 24, 5: 18, 6: 42, 7: 42, 8: 22, 9: 28, 10: 18, 11: 20, 12: 20, 13: 28})
...
def _build_ic_memo(wb: Workbook, facts: SourceFacts) -> None:
    ...
    final_col = _final_period_col(facts)
    sections = [
        ("Recommendation", f"=IF(AND('Valuation'!{final_col}32>=2,'Valuation'!{final_col}25>=0.9,'Pricing'!{final_col}21=\"pass\"),\"Proceed subject to DD gates\",\"Do not circulate externally until valuation, pricing, and evidence gates are cleared\")"),
        ...
        ("Valuation and return", f"=\"Selected EV midpoint: JPY \"&ROUND('Valuation'!{final_col}27/10^6,0)&\"M; supportability: \"&TEXT('Valuation'!{final_col}25,\"0.0x\")&\"; MOIC: \"&TEXT('Valuation'!{final_col}32,\"0.0x\")&\"; IRR: \"&TEXT('Valuation'!{final_col}33,\"0%\")"),
        ("Price / terms stance", f"=\"Target price range: JPY \"&ROUND('Valuation'!{final_col}26/10^6,0)&\"M-\"&ROUND('Valuation'!{final_col}28/10^6,0)&\"M EV; pricing gate: \"&'Pricing'!{final_col}21&\"; exit signal: \"&'Exit Waterfall'!N7"),
