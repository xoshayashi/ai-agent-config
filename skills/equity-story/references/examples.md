...
東証グロースIPOは headline-number(上場時の成長性)に立脚するため、市場規模は
`market-sizing.md` の規律で組む — TAMを独立ルートで三角測量し、SAMを予算源泉から
積んで重複控除し、SOMを4シナリオで類似企業と照合する。*以下の数値は本書時点の
前提値・推計(`[estimate]`)であり、上場準備で一次統計により確定する。*

**TAM — 独立3ルートの三角測量。** 賃貸管理業務全体(管理SaaS+事務+家賃決済)
が再設計の対象。

- ルートA(管理戸数 × 戸あたり支出): 全国の管理委託戸数 約1,500万戸 × 管理関連
  ソフト・サービスの戸あたり年支出 約2万円 ≒ **約3,000億円** `[estimate]`
- ルートB(事務人件費 × デジタル代替率): 賃貸管理業の事務人件費総額 約1兆円 ×
  デジタル代替可能率 約35% ≒ **約3,500億円** `[estimate]`
- ルートC(流通家賃 × テイクレート): 管理家賃の年間流通額 約10兆円 × 低位の決済
  テイクレート 約3.5% ≒ **約3,500億円** `[estimate]`
- 入力を共有しない3ルートが **約3,000〜3,500億円** の同オーダーに収束する。

**SAM — 予算源泉別、Gross→Net。** 市場名の足し上げでなく、管理会社の予算プール
から積む。

- 管理SaaS予算プール 約500億円 + 事務人件費削減枠プール 約600億円 + 家賃決済
  手数料原資プール 約300億円 = **Gross SAM 約1,400億円** `[estimate]`
- 同一管理会社が複数プールから支出する重複を約15%控除 → **Net SAM 約1,200億円**
  `[derived]`。うちSaaS利用料のみの serviceable layer は 30,000社 × 約67万円 ≒
  約200億円で、決済・隣接サービスが残りを構成する。

**SOM — 5年目、4シナリオ。** 各シナリオで導入社数・ARPA・決済アタッチ率の
ドライバー前提を変える。

| シナリオ | 導入社数 | ARPA(SaaS+決済) | 5年目SOM | Net SAM浸透率 | TAM浸透率 |
|---|---|---|---|---|---|
| Conservative | 6,000社 | 80万円 | 約48億円 | 約4% | 約1.4% |
| Base | 9,000社 | 95万円 | 約85億円 | 約7% | 約2.4% |
| Target | 13,000社 | 115万円 | 約150億円 | 約12% | 約4.3% |
| Upside | 18,000社 | 130万円 | 約235億円 | 約20% | 約6.7% |

クロスチェック: 国内バーティカルSaaSの上場企業は、上場時点で自社SAM比おおむね
6〜10%の浸透 `[estimate]`。Base の約7%はその範囲内、Upside の約20%はそれを超える
強気ケースとして明示する。

> Annotation: 成長可能性資料 項目② 市場環境 に対応。ヤモリは東証グロースIPO =
> headline-number argument のため、Example 1(US Series B — ボトムアップで十分)
> より深い `market-sizing.md` の規律 — TAM三角測量・予算源泉別SAM・4シナリオSOM・
> 類似企業クロスチェック — を実演している。数値はすべて例示であり、実際の文書では
> 各ルートの入力を一次統計から独立に導出すること。
...
- **決済の収益化** — 出来高請求に連動した決済のアタッチ率が、上場後おおむね2年
  ...
  物語は誤りであり、評価は決済を除いたSaaS単体の serviceable layer(約200億円)へ
  retreat する。
