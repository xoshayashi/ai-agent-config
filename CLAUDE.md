...
The orchestrator implements a Codex-as-hub multi-LLM loop driven by completion keywords (`[[SPEC_DONE]]` → spec review → `[[IMPLEMENTATION_DONE]]` → verification → `[[VERIFICATION_DONE]]` → `[[TASK_DONE]]`). The lifecycle and stop conditions live in `instructions/HOOKS.md`; do not duplicate them elsewhere.
