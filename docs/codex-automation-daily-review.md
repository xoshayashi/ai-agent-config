...
日次 LLM 履歴 instruction review のスケジューラは Codex App Automations
です。launchd は使いません。定期実行の schedule、prompt、run history、
sandbox mode、model、findings は Codex app 側で一体管理します。
...
- 「self-review して」「eval/Iteration を回して」「品質が足りない」のような
  反復プロンプトを、完了宣言前の self-review と改善 Iteration に翻訳できている
