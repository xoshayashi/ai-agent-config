...
| Terminal.app に `setup.sh:` で始まる window が開く | `displaylink` / `docker-desktop` / `google-drive` / `zoom` / `pmset` など、管理者権限が必要な処理だけを通常の Terminal で実行しています。 | Terminal の `sudo` プロンプトに macOS のパスワードを入力し、完了まで待ちます。 |
