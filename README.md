...
Claude Code / Codex / Gemini CLI / GitHub Copilot CLI で共通の Instructions・Skills・Hooks を配布するためのリポジトリです。
**現在は 4 CLI のグローバル設定**を配布し、この repo 自体にも Copilot 用の repo-level instructions を同梱しています。
...
> 注意: `scripts/setup.sh` は **Claude Code / Codex / Gemini CLI / GitHub Copilot CLI のグローバル設定**を自動配置します。
> さらにこの repo では `.github/copilot-instructions.md` を tracked file として持ち、Copilot が repo context でも同じ方針を読みやすくしています。
>
...
- 既定: `~/.agents/skills`
- Copilot CLI 用にも同じ Skill を `~/.copilot/skills` へリンクします
...
Hook 本体はこのリポジトリで管理し、各 CLI の公式グローバル設定フォルダー内に `hooks/` リンクを作って参照します。

| CLI | Hook runtime link |
|---|---|
| Claude Code | `~/.claude/hooks -> <this repo>/hooks` |
| Codex | `~/.codex/hooks -> <this repo>/hooks` |
| Gemini CLI | `~/.gemini/hooks -> <this repo>/hooks` |
| GitHub Copilot CLI | `~/.copilot/hooks -> <this repo>/hooks` |

共有 state や backup は既定で `~/.ai-agent-config/` に保存します。
shared state や backup の正本は `~/.ai-agent-config/` です。

## 前提条件

1. ターミナルを起動できること
2. **Claude Code / Codex / Gemini CLI / GitHub Copilot CLI を全てインストール済み**であること
3. **4つ全てでログイン済み**であること
4. `git` と `python3` が使えること
5. `trash` コマンド（macOS は `brew install trash`、Linux は `trash-cli` パッケージ）

`scripts/setup.sh` は既定で 4 CLI の存在チェックを行います（`AI_AGENT_REQUIRE_LLM_CLIS=0` で無効化可能）。
...
```sh
AI_AGENT_ASSUME_YES=1 sh /path/to/ai-agent-config/scripts/enable-auto-permission.sh
```
...
- 管理レイヤーは **ユーザーグローバル1層**に統一（`~/.claude`, `~/.codex`, `~/.gemini`, `~/.copilot`）
- プロジェクト層への自動 Hook 配布は廃止
- 安全性重視 Hook（`safe_delete_guard.py`）は既定ON
- 共通 Hook（`subprocess_check.py`、旧 `self_workflow.py` 互換 shim あり）は **Codex / Claude Code / Gemini CLI / GitHub Copilot CLI の managed event に登録済み**
- GitHub Copilot CLI も同じ canonical instructions / managed Hook / self-workflow / global setup の対象です
- `subprocess_check.py` は Hook 境界で同じ CLI を非対話サブプロセスとして呼び、次の具体的な 1 手か `STATUS: complete` を返させます
...
- Copilot 向けの global entrypoint は [instructions/COPILOT.md](instructions/COPILOT.md)、repo-level entrypoint は [instructions/.github/copilot-instructions.md](instructions/.github/copilot-instructions.md) を正本にしています
- `scripts/setup.sh` / `scripts/health-check.sh` / Hook runtime も Copilot を含めて扱います。repo 直下の [.github/copilot-instructions.md](.github/copilot-instructions.md) は、この repo 自体で Copilot を使うときの tracked entrypoint です
