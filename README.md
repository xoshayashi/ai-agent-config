...
| GitHub Copilot | `<repo>/.github/copilot-instructions.md`（repo-local、手動管理） |
...
> Copilot の指示はリポジトリ単位で扱う前提のため、必要な場合は対象リポジトリ側で `.github/copilot-instructions.md` を明示的に配置してください。
>
> このリポジトリでは、CLI や `/init` 系コマンドがローカルに生成しうる `/AGENTS.md`、`/CLAUDE.md`、`/GEMINI.md`、`/.github/copilot-instructions.md` は **canonical ではありません**。正本は `instructions/` 配下にあり、これらのローカル生成物は `.gitignore` で無視します。
...
GitHub Copilot を使うリポジトリなら、`instructions/.github/copilot-instructions.md` を正本として `.github/copilot-instructions.md` の扱いも案内して。
...
- GitHub Copilot は同じ canonical instructions を参照できますが、現行実装では **managed Hook / self-workflow / global setup の対象外**
